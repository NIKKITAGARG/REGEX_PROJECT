...
export default function Finalinvoice() {
  ...
  console.log("prevdata=",prev_data);
  ...
  const profiledata=async()=>{
    ...
    setAddress1(profobj.address_line_1);
    setAddress2(profobj.address_line_2);
    setContact(profobj.contact);
    setGst(profobj.gst_in);
    setPan(profobj.pan);
    setAccount(profobj.account);
    setBankAddress(profobj.bankaddress);
    setIfsc(profobj.ifsc);
    setBeneficiary(profobj.beneficiary);
 }
 useEffect(()=>{
   profiledata();
 },[])
...
<div className="container border border-dark border-top-0 border-bottom-0 mt-0 w-auto">
  <div className="row row-cols-2 ">
    ...
    <div className="col  p-1 fs-bold border border-dark">
        <div className='fw-bold '>GSTIN:<div className='d-inline mx-3 text-muted'>{prev_data.gst_in}</div></div>
        ...
        <div className='fw-bold '>PH:<div className='d-inline mx-3 text-muted'>{prev_data.zpid}</div></div>
...
<div className="container border border-dark border-top-0 border-bottom-0 mt-3 w-auto">
  <div className="row row-cols-1 ">
    ...
    <div className="col border border-dark  fs-bold">
        <div className=' '>Beneficiary Name as per Bank A/c: <span className='text-muted fw-bold'>{beneficiary}</span></div>
        <div className=' '>Account Number - <span className='text-muted fw-bold'>{account}</span></div>
        <div className=' '>IFSC Code: <span className='text-muted fw-bold'>{ifsc}</span></div>
        <div className=' '>Bank Address: <span className='text-muted fw-bold'>{bankaddress}</span></div>
    </div>
...
<div className="container  border border-dark border-top-0 border-bottom-0 mt-3 w-auto fs-xs-5 ">
  ...
  <div className="row row-cols-2 row-cols-lg-5">
    <div className="col-5 border border-dark p-1 fw-bold ">
        <div className=' '>Trainer name:<div className='d-inline mx-3 text-muted'>{prev_data.name_of_the_trainer}</div></div>
